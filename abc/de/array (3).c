...
{	
	...
	printf("a: %p\n", a); // FF22 = 65314 i.e. hexadecimal（16进制） = decimal
	printf("b: %p\n", b); // FF18 = 65304
	printf("c: %p\n", c); // FF0E = 65294
...
int main(void)
{
	...
	char str3[10]={0}; // no initialisation
	str3[0] = 'a';
	str3[1] = '\0';
...
/*gets()的用法，gets() allows you to
input any length of
characters.
Hence, there might be a
buffer overflow. */
#include<stdio.h>
int main()
{
...
int main(void)
{
	...
	str[1]='\0';
	int l=0;
	l=strlen(str); //strlen所作的是一个计数器的工作，它从内存的某个位置（可以是字符串开头，中间某个位置，甚至是某个不确定的内存区域）开始扫描，直到碰到第一个字符串结束符'\0'为止，然后返回计数器值(长度不包含'\0')
	printf("%s and %d\n",str,l);
	return 0;
}